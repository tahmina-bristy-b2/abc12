...
class ECMEDoctorPreviewScreen extends StatefulWidget {
  ...
  @override
  State<ECMEDoctorPreviewScreen> createState() =>
      _ECMEDoctorPreviewScreenState();
}
...
class _ECMEDoctorPreviewScreenState extends State<ECMEDoctorPreviewScreen> {
  ...
  dynamicTotalCalculation() {
    totalAmount = ECMEServices()
        .dynamicTotalCalculation(widget.eCMESubmitDataModel)["total_amount"];
    eCMEAmount = ECMEServices()
        .dynamicTotalCalculation(widget.eCMESubmitDataModel)["eCME_amount"];
    setState(() {});
  }
    ...
    return Scaffold(
      ...
      body: SafeArea(
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ((widget.eCMESubmitDataModel.eCMEType == "Intern Reception") ||
                      (widget.eCMESubmitDataModel.eCMEType == "Society"))
                  ? const SizedBox()
                        ...
                        children: [
                          Expanded(
                            child: Padding(
                              ...
                              child: SizedBox(
                                height:
                                    widget.eCMESubmitDataModel.docList.length *
                                        20,
                                width: 150,
                                child: ListView.builder(
                                    primary: false,
                                    itemCount: widget
                                        .eCMESubmitDataModel.docList.length,
                                    itemBuilder: (context, index) {
                                      return Padding(
                                        padding: const EdgeInsets.only(top: 4),
                                        child: Text(
                                          "${index + 1}. ${widget.eCMESubmitDataModel.docList[index].docName}|${widget.eCMESubmitDataModel.docList[index].degree}|${widget.eCMESubmitDataModel.docList[index].specialty}",
              ...
              RowForCMEPreview(
                title: 'CME Type',
                value: widget.eCMESubmitDataModel.eCMEType,
                isBold: true,
              ),
              RowForCMEPreview(
                title: 'Meeting Date',
                value: widget.eCMESubmitDataModel.meetingDate,
                isBold: false,
              ),
              RowForCMEPreview(
                title: 'Doctor Category',
                value: widget.eCMESubmitDataModel.doctorCategory,
                isBold: false,
              ),
              widget.eCMESubmitDataModel.doctorCategory == "Institution"
                  ? RowForCMEPreview(
                      title: 'Institute Name',
                      value: widget.eCMESubmitDataModel.institutionName,
                      isBold: false,
                    )
                  : const SizedBox(),
              widget.eCMESubmitDataModel.doctorCategory == "Institution"
                  ? RowForCMEPreview(
                      title: 'Department',
                      value: widget.eCMESubmitDataModel.departament,
                      isBold: false,
                    )
                  : const SizedBox(),
              RowForCMEPreview(
                title: 'Meeting Venue',
                value: widget.eCMESubmitDataModel.meetingVanue,
                isBold: false,
              ),
              RowForCMEPreview(
                title: 'Meeting Topic',
                value: widget.eCMESubmitDataModel.meetingTopic,
                isBold: false,
              ),
              RowForCMEPreview(
                title: 'Probable Speaker Name & Designation',
                value: widget.eCMESubmitDataModel.speakerName,
                isBold: false,
              ),
              RowForCMEPreview(
                title: 'Pay Mode',
                value: widget.eCMESubmitDataModel.payMode,
                isBold: false,
              ),
              RowForCMEPreview(
                title: 'Pay to',
                value: widget.eCMESubmitDataModel.payTo,
                isBold: false,
              ),
              RowForCMEPreview(
                title: 'Total Numbers of participants',
                value: widget.eCMESubmitDataModel.numberOfParticipant,
                isBold: false,
              ),
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    PreviewBreakdownRowWidget(
                      ...
                      value: widget.eCMESubmitDataModel.doctorsCount,
                    ),
                    PreviewBreakdownRowWidget(
                      title: '2.  Intern Doctors',
                      value: widget.eCMESubmitDataModel.internDoctor,
                    ),
                    PreviewBreakdownRowWidget(
                      title: '3.  DMF / RMP Doctors',
                      value: widget.eCMESubmitDataModel.dMFDoctors,
                    ),
                    PreviewBreakdownRowWidget(
                      title: '3.  Nurses / Staffs',
                      value: widget.eCMESubmitDataModel.nurses,
                    ),
                    PreviewBreakdownRowWidget(
                      title: '4.  SKF Attendees ',
                      value: widget.eCMESubmitDataModel.skfAttendance,
                    ),
                    PreviewBreakdownRowWidget(
                      title: '6.  Others',
                      value: widget.eCMESubmitDataModel.othersParticipants,
                    ),
                  ],
                ),
              ),
              RowForCMEPreview(
                title: 'Total Budget',
                value: widget.eCMESubmitDataModel.totalBudget,
                isBold: true,
              ),
              RowForCMEPreview(
                title: 'Cost Per doctor',
                value: widget.eCMESubmitDataModel.costPerDoctor,
                isBold: true,
              ),
              const RowForCMEPreview(
              ...
              Container(
                ...
                child: Column(
                  children: [
                    PreviewBreakdownRowWidget(
                      title: '1.  Hall rent',
                      value: widget.eCMESubmitDataModel.hallRentAmount,
                    ),
                    PreviewBreakdownRowWidget(
                      title: '2.  Food Expense',
                      value: widget.eCMESubmitDataModel.foodExpense,
                    ),
                    PreviewBreakdownRowWidget(
                      title: '3.  Gift / Souvenir',
                      value: widget.eCMESubmitDataModel.giftCost,
                    ),
                    PreviewBreakdownRowWidget(
                      title:
                          '4.  Stationaires and Others (Pen, Photocopies etc)',
                      value: widget.eCMESubmitDataModel.stationaries,
                    ),
                  ],
                ),
              ),
              ...
              BrandDetailsShowWidget(
                ...
                rxOrSalesTile:
                    widget.eCMESubmitDataModel.eCMEType == "RMP Meeting"
                        ? "Sales Qty"
                        : "Rx Objective / Day",
  ...
  eDsrSubmit() async {
    ...
    if (data["status"] == "Success") {
      ...
      if ((widget.eCMESubmitDataModel.eCMEType == "Intern Reception") ||
          (widget.eCMESubmitDataModel.eCMEType == "Society")) {
        if (!mounted) return;
        Navigator.pop(context);
        Navigator.pop(context);
