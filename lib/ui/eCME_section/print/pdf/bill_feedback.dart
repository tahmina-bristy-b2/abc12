...
import 'package:MREPORTING_OFFLINE/ui/eCME_section/print/util/feedback_utils.dart';
...
class _BillfeedbackPrintState extends State<BillfeedbackPrint> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          'Feedback Letter(${widget.dataListPrint.sl})',
      ...
      body: PdfPreview(
        ...
        maxPageWidth: 700,
        actions: acitons,
        onPrinted: (BuildContext context) => BillFeedbackUtils()
            .showPrintedToast(context, widget.wholeData, widget.dataListPrint,
                widget.editedData),
        onShared: (BuildContext context) => BillFeedbackUtils().showSharedToast(
            context, widget.wholeData, widget.dataListPrint, widget.editedData),
        build: (PdfPageFormat format) => BillFeedbackUtils().generatePdf(
            format, widget.wholeData, widget.dataListPrint, widget.editedData),
      ),
    );
  }
}
