...
import 'package:MREPORTING_OFFLINE/ui/eCME_section/print/util/proposal_bill_utils.dart';
...
class _ProposalBillPdfScreenState extends State<ProposalBillPdfScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          'Proposal Bill PDF(${widget.dataListPrint.sl})',
      ...
      body: PdfPreview(
        ...
        maxPageWidth: 700,
        actions: acitons,
        onPrinted: (BuildContext context) => ProposalBillPrintUtil()
            .showPrintedToast(context, widget.wholeData, widget.dataListPrint),
        onShared: (BuildContext context) => ProposalBillPrintUtil()
            .showSharedToast(context, widget.wholeData, widget.dataListPrint),
        build: (PdfPageFormat format) => ProposalBillPrintUtil()
